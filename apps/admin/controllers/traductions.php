...
class traductionsController extends bootstrap
{
	...
	function _default()
	{
		...
		if(isset($_POST['form_import_traduction']))
		{
			if(isset($_FILES['csv']) && $_FILES['csv']['name'] != '')
			{
				...
				{					
					...
					// Initialisation de la 1čre ligne du csv
					$row = 1;
			...
			// Renvoi sur la liste des traductions avec la section préchargée
		...
		// Si une section est passée on recupere la liste des nom
		if(isset($this->params[0]) && $this->params[0] != '')
		{
			$this->lNoms = $this->ln->selectTexts($this->params[0]);	
		}
		
		// Si un nom est passée on recupere la liste des trads
