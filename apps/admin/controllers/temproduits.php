...
class temproduitsController extends bootstrap
{
	var $Command;
	...
	function _default()
	{
		...
		$this->lTemplate = $this->templates->select('type = 1','name ASC');
		...
		if(isset($_POST['form_edit_template']))
		{
			...
			header('Location:'.$this->lurl.'/temproduits');
			die;
		}
		...
		if(isset($_POST['form_add_template']))
		{
			...
			// Creation des elements obligatoire par defaut
			$this->elements->id_template = $this->templates->id_template;
			$this->elements->name = 'Meta title';
			$this->elements->slug = $this->bdd->generateSlug($this->elements->name);			
			$this->elements->ordre = 0;
			$this->elements->type_element = 'Texte';
			$this->elements->status = 1;
			$this->elements->create();
			
			$this->elements->id_template = $this->templates->id_template;
			$this->elements->name = 'Meta description';
			$this->elements->slug = $this->bdd->generateSlug($this->elements->name);			
			$this->elements->ordre = 1;
			$this->elements->type_element = 'Textearea';
			$this->elements->status = 1;
			$this->elements->create();
			
			$this->elements->id_template = $this->templates->id_template;
			$this->elements->name = 'Meta keywords';
			$this->elements->slug = $this->bdd->generateSlug($this->elements->name);			
			$this->elements->ordre = 2;
			$this->elements->type_element = 'Textearea';
			$this->elements->status = 1;
			$this->elements->create();
			
			$this->elements->id_template = $this->templates->id_template;
			$this->elements->name = 'Nom du produit';
			$this->elements->slug = $this->bdd->generateSlug($this->elements->name);			
			$this->elements->ordre = 3;
			$this->elements->type_element = 'Nom Produit';
			$this->elements->status = 1;
			$this->elements->create();
			
			$this->elements->id_template = $this->templates->id_template;
			$this->elements->name = 'Description courte';
			$this->elements->slug = $this->bdd->generateSlug($this->elements->name);			
			$this->elements->ordre = 4;
			$this->elements->type_element = 'Textearea';
			$this->elements->status = 1;
			$this->elements->create();
			
			$this->elements->id_template = $this->templates->id_template;
			$this->elements->name = 'Description longue';
			$this->elements->slug = $this->bdd->generateSlug($this->elements->name);			
			$this->elements->ordre = 5;
			$this->elements->type_element = 'Texteditor';
			$this->elements->status = 1;
			$this->elements->create();
	...
	function _elements()
	{
		...
		if(isset($_POST['form_add_bloc']))
		{
			...
			// Renvoi sur l'edition du template
			header('Location:'.$this->lurl.'/temproduits/elements/'.$_POST['id_template']);
			die;
		}
		...
		if(isset($_POST['form_edit_element']))
		{
			...
			header('location:'.$this->lurl.'/temproduits/elements/'.$this->elements->id_template);
			die;
		}
		...
		if(isset($this->params[1]) && $this->params[1] != '')
		{
			switch($this->params[1])
			{
				case 'up':
					...
					header('location:'.$this->lurl.'/temproduits/elements/'.$this->params[0]);
					die;
