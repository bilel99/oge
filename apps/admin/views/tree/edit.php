...
<div id="contenu">
    ...
    <div class="btnDroite" style="margin-top:5px"><a href="<?=$this->lurl?>/tree/copyPage/<?=$this->params[0]?>" class="btn_link thickbox">Dupliquer la page</a></div> 
    <?php 
                ...
                <fieldset>
                    ...
                    <div class="gauche">                   
                        ...
                        <table class="form">
                            <tr>
                                ...
                                <td>
                                    <select name="id_parent_<?=$key?>" id="id_parent_<?=$key?>" onchange="setNewIdParent(this.value);" class="select">
                                        ...
                                        {
                                            echo '<option value="'.$tree['id_tree'].'"'.($this->tree->id_parent == $tree['id_tree']?' selected="selected"':'').'>'.$tree['title'].'</option>';
                                        }
                                        ?>
                                    </select>
                                </td>
                            ...
                            <tr>
                                ...
                                <td>
                                    <select name="id_template_<?=$key?>" id="id_template_<?=$key?>" onchange="if(confirm('Voulez-vous vraiment changer ce template ?\nLe contenu existant de cette page sera d\351finitivement \351ffac\351.')){ window.location.href = '<?=$this->lurl?>/tree/edit/<?=$this->tree->id_tree?>/<?=$key?>/' + this.value; } else { this.value = '<?=$this->tree->id_template?>'; }" class="select">
                                        ...
                                        {
                                            echo '<option value="'.$template['id_template'].'"'.($this->tree->id_template == $template['id_template']?' selected="selected"':'').'>'.$template['name'].'</option>';
                                        }
                                        ?>
                                    </select>
                                </td>
                            ...
                            <?
							if($this->tree->img_menu != '')
							{
								list($width,$height) = @getimagesize($this->surl.'/var/images/'.$this->tree->img_menu);
							?>
