...
class upload {
	...
	// Taille max des fichiers pouvant ętres uploadés en octets (rappel : 1Ko -> 1024 octets)
	// Par defaut la valeur est de 8Mo
	private	$taille_max = 8192000;
	
	// Tableau des extensions que l'on autorise a etre uploadé
	private $ext_valides = array('jpg','JPG','png','PNG','gif','GIF','pdf','PDF','doc','DOC','xls','XLS','ppt','PPT','csv','CSV','swf','SWF');
	
	// Constructeur
	public function __construct()
	{
		// Je préfčre définir mes paramčtres d'upload dans la méthode doUpload()
		// comme ça je peux construire mon objet Upload() avant męme de recevoir un fichier d'un formulaire
	...
	{
		$name_file = strtr($name_file, 'ŔÁÂĂÄĹÇČÉĘËĚÍÎĎŇÓÔŐÖŮÚŰÜÝŕáâăäĺçčéęëěíîďđňóôőöůúűüý˙', 'AAAAAACEEEEIIIIOOOOOUUUUYaaaaaaceeeeiiiioooooouuuuyy');
	...
	// ************************************************ //
	// Fonction qui récupére le nom du fichier uploader //
	// ************************************************ //
	...
	// ************************************************* //
	// Fonction qui récupére le type du fichier uploader //
	// ************************************************* //
	...
	// ***************************************************** //
	// Fonction qui récupére l'extension du fichier uploader //
	// ***************************************************** //
	...
	// ************************************************** //
	// Fonction qui récupére le poids du fichier uploader //
	// ************************************************** //
		...
		if(is_uploaded_file($this->uploadedFile) && !empty($this->uploadedFile))
		{
			...
			if($this->uploadedFileSize < $this->taille_max)
			{
				...
				if(in_array($this->uploadedFileExtension,$this->ext_valides))
				{
					...
					{
						sleep(1); // Temporisation du script pour eviter d'avoir une image avec le męme nom (time() s'increment toutes les secondes)
