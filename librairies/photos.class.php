...
class photos
{
	...
	// On charge la classe avec le spath et le surl
	public function photos($params)
	{
		$this->spath = $params[0];
		$this->surl = $params[1];
		
		// Gestion des formats d'image que l'on souhaite sur le site	
		$this->formats = array(
	...
	public function resizeImageWH($w_max,$h_max,$type='',$cadre_fixe=false)
	{
		...
		if($this->uploadedFileExtension == 'jpg' || $this->uploadedFileExtension == 'JPG')
		{
			if($cadre_fixe)
			{
				// PREPARATION DU FICHIER DE L'IMAGE FINALE
